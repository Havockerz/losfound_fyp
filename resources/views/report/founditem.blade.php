    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <a href="{{ route('report.foundindex') }}"
                    class="inline-flex items-center px-4 py-2 bg-green-300 border border-black rounded-md font-semibold text-xs text-black uppercase tracking-widest hover:bg-green-400 hover:shadow-md focus:bg-green-400 active:bg-green-500 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-sm">
                            ...
                            @forelse ($items as $item)
                                                            <tr class="bg-white border-b hover:bg-gray-50 transition">
                                                                <td class="px-6 py-4">
                                                                    ...
                                                                    @else
                                                                        <div
                                                                            class="w-12 h-12 bg-gray-100 flex items-center justify-center rounded-lg text-[10px] text-gray-400">
                                                                            No Image</div>
                                                                    @endif
                                                                </td>
                                                                ...
                                                                <td class="px-6 py-4">{{ \Carbon\Carbon::parse($item->reported_date)->format('d M Y') }}
                                                                </td>
                                                                <td class="px-6 py-4">
                                                                    <span
                                                                ...
                                                                <td class="px-6 py-4">
                                                                    <div class="flex items-center justify-center gap-3">
                                                                        ...
                                                                        @if(auth()->user()->role === 'admin' || auth()->id() === $item->user_id)
                                                                            <a href="{{ route('report.foundedit', $item->id) }}" class="inline-flex items-center px-3 py-1 
